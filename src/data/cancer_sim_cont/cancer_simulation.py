                ...
                for projection_time in range(0, projection_horizon):
                    ...
                    counterfactual_cancer_volume[current_t + 1] = counterfactual_cancer_volume[current_t] *\
                        (1 + rho * np.log(K / (counterfactual_cancer_volume[current_t] + 1e-07) + 1e-07) -
                         ...
                         noise[current_t + 1])
                    # 这一句的作用是将 counterfactual_cancer_volume[current_t + 1] 限制在 0 和 TUMOUR_DEATH_THRESHOLD 之间，防止出现负数或超过最大肿瘤阈值。
                    counterfactual_cancer_volume[current_t + 1] = np.clip(
                        counterfactual_cancer_volume[current_t + 1], 0, TUMOUR_DEATH_THRESHOLD)
