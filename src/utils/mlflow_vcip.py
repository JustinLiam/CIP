...
class VCIPMlflowTracker:
    ...
    def log_ct_epoch_metrics(
        self,
        epoch: int,
        *,
        train_pred: float,
        train_align: float,
        train_ex: Mapping[str, float],
        val_pred: float,
        val_align: float,
        val_ex: Mapping[str, float],
        es_score: float,
    ) -> None:
        """Mirror train_ct.py per-epoch console diagnostics."""
        if not self.enabled or not self._active:
            return
        metrics: Dict[str, float] = {
            "train/loss_pred": float(train_pred),
            "train/align": float(train_align),
            "train/L1": float(train_ex.get("mean_l1", float("nan"))),
            "train/L1_anchor": float(train_ex.get("mean_l1_anchor", float("nan"))),
            "train/mae_uw_norm": float(train_ex.get("mae_uw_norm", float("nan"))),
            "train/mae_uw_uns": float(train_ex.get("mae_uw_uns", float("nan"))),
            "train/mean_dyn": float(train_ex.get("mean_dyn", float("nan"))),
            "train/w_ess_frac": float(train_ex.get("w_ess_frac", float("nan"))),
            "train/w_std": float(train_ex.get("w_std", float("nan"))),
            "val/loss_pred": float(val_pred),
            "val/align": float(val_align),
            "val/L1": float(val_ex.get("mean_l1", float("nan"))),
            "val/L1_anchor": float(val_ex.get("mean_l1_anchor", float("nan"))),
            "val/mae_uw_norm": float(val_ex.get("mae_uw_norm", float("nan"))),
            "val/mae_uw_uns": float(val_ex.get("mae_uw_uns", float("nan"))),
            "val/mean_dyn": float(val_ex.get("mean_dyn", float("nan"))),
            "val/w_ess_frac": float(val_ex.get("w_ess_frac", float("nan"))),
            "val/w_std": float(val_ex.get("w_std", float("nan"))),
            "val/es_score": float(es_score),
        }
        if train_ex.get("align_pre") is not None:
            metrics["train/align_pre"] = float(train_ex["align_pre"])
        self.log_metrics(metrics, step=int(epoch))
